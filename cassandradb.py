...
def insert_data():
    ...
    results=session.execute("select max(id) as id from student.details")
    id=results.one().id
    # print(id)
    stmt=session.prepare("INSERT INTO student.details (id,name,prn,year,email) values (?,?,?,?,?)")
...
def read_data():
    ...
    rows=session.execute("Select * from student.details")
    for row in rows:
...
def delete_data():
    ...
    id=results.one().id
    print(id)
    stmt=session.prepare("DELETE from student.details where id=(?)")
